...
async def test_timeline_v2_fresh():
    ...
    # Use a short test video to minimize cost
    test_url = "https://www.youtube.com/watch?v=yU4xvJRx4Yc"  # Short test video
    ...
    try:
        ...
        logger.info(f"\n{'='*40}")
        logger.info(f"Processing FRESH video (no cache): {test_url}")
        video = await retriever.process_url(test_url)
        ...
        if video:
            ...
            # Process collection with just this video to test Timeline v2.0
            logger.info(f"\n{'='*60}")
            logger.info("Testing Timeline v2.0 with single fresh video...")
            ...
            collection = await processor.process_video_collection(
                ...
                collection_title="Single Video Timeline v2.0 Test"
            )
