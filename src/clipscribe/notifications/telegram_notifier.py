...
Tap below to review:"""
        ...
        keyboard = [[InlineKeyboardButton("📱 Review Draft", url=draft_url)]]
        ...
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    ...
                    logger.error("Rate limited after 3 attempts")
    ...
    async def notify_processing_started(self, title: str):
        ...
        try:
            await self.bot.send_message(chat_id=self.chat_id, text=f"⏳ Processing: {title}...")
        except Exception as e:
    ...
    async def notify_error(self, title: str, error: str):
        ...
        try:
            await self.bot.send_message(
                chat_id=self.chat_id, text=f"❌ Failed: {title}\n\nError: {error}"
            )
        except Exception as e:
