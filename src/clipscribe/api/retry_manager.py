...
class RetryManager:
        ...
        try:
            ...
            if data:
                ...
                return RetryState(
                    ...
                    last_attempt=datetime.fromisoformat(state_dict['last_attempt']) if state_dict.get('last_attempt') else None,
                    next_retry=datetime.fromisoformat(state_dict['next_retry']) if state_dict.get('next_retry') else None,
                    failures=state_dict.get('failures', 0),
                    circuit_breaker_tripped=state_dict.get('circuit_breaker_tripped', False),
                    circuit_breaker_until=datetime.fromisoformat(state_dict['circuit_breaker_until']) if state_dict.get('circuit_breaker_until') else None
                )
    ...
    def _save_retry_state(self, operation_id: str, state: RetryState):
        ...
        try:
            ...
            state_dict = {
                ...
                'circuit_breaker_until': state.circuit_breaker_until.isoformat() if state.circuit_breaker_until else None
            }
    ...
    def _check_circuit_breaker(self, operation_id: str, state: RetryState) -> bool:
        ...
        if state.failures >= self.config.circuit_breaker_threshold:
            ...
            state.circuit_breaker_until = datetime.now() + timedelta(seconds=self.config.circuit_breaker_timeout)
            logger.warning(f"Circuit breaker tripped for {operation_id}, resetting at {state.circuit_breaker_until}")
            return False
    ...
    async def execute_with_retry(
        ...
        while state.attempt < self.config.max_attempts:
            try:
                ...
                if state.failures > 0:
                    state.failures = 0
                    logger.info(f"Operation {operation_id} recovered after {state.attempt} attempts")
                ...
                if state.attempt >= self.config.max_attempts:
                    logger.error(f"Operation {operation_id} failed after {state.attempt} attempts: {e}")
                ...
                logger.warning(f"Operation {operation_id} attempt {state.attempt} failed: {e}. Retrying in {delay:.1f}s")
    ...
    def _move_to_dead_letter(self, operation_id: str, error: Exception, state: RetryState):
        ...
        try:
            ...
            self.redis.set(dead_letter_key, json.dumps(dead_letter_data), ex=self.config.dead_letter_ttl)
...
def should_retry_network_error(error: Exception) -> bool:
    ...
    return (
        ...
        any(msg in error_msg for msg in retryable_messages)
    )
