...
class WhisperXTranscriber:
    ...
    async def transcribe_audio(
        ...
        try:
            ...
            raise ImportError(
                "WhisperX not installed. Run: poetry add whisperx"
            )
    ...
    def _transcribe_sync(
        ...
        logger.info(f"✓ WhisperX complete: {len(word_timestamps)} words, {len(speaker_segments)} speakers")
...
async def test_whisperx():
    ...
    if len(sys.argv) < 2:
        print("Usage: poetry run python -m clipscribe.transcribers.whisperx_transcriber <audio_file>")
        return
    ...
    for speaker in result.speaker_segments:
        print(f"  {speaker['speaker']}: {speaker['total_time']:.1f}s ({speaker['segments']} segments)")
    print("="*80)
