...
class GeoIntExporter:
    ...
    def export_kml(
        self, telemetry: List[Dict], events: List[Dict] = None, filename: str = "mission.kml"
    ):
        """
        ...
        for p in telemetry:
            if p.get("SensorLatitude") is not None:
                ...
                sensor_coords.append(
                    f"{p['SensorLongitude']},{p['SensorLatitude']},{p.get('SensorTrueAltitude', 0)}"
                )

        if sensor_coords:
            kml.append(
                self._create_linestring(
                    "Flight Path", "#dronePath", sensor_coords, altitude_mode="absolute"
                )
            )
        ...
        for p in telemetry:
            if p.get("FrameCenterLatitude") is not None:
                target_coords.append(
                    f"{p['FrameCenterLongitude']},{p['FrameCenterLatitude']},{p.get('FrameCenterElevation', 0)}"
                )

        if target_coords:
            kml.append(
                self._create_linestring(
                    "Target Track", "#targetPath", target_coords, altitude_mode="clampToGround"
                )
            )
    ...
    def export_interactive_map(
        self, telemetry: List[Dict], events: List[Dict] = None, filename: str = "mission_map.html"
    ):
        """
