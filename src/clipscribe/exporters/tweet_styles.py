...
class TweetStyleGenerator:
    ...
    async def generate_all_styles(
        ...
        """
        entity_names = [e.get('name') if isinstance(e, dict) else getattr(e, 'name', '') for e in entities[:5]]
        rel_texts = []
        ...
        # Generate each style
        styles['analyst'] = await self._generate_analyst_style(title, summary[:500], entity_names, rel_texts)
        styles['alarm'] = await self._generate_alarm_style(title, summary[:500], entity_names, rel_texts)
        styles['educator'] = await self._generate_educator_style(title, summary[:500], entity_names, rel_texts)
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                data = {
                    "model": "grok-4-0709",
                    "messages": [
                        {"role": "system", "content": "You write engaging X posts in specific styles."},
