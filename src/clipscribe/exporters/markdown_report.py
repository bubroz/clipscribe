...
def generate_markdown_report(
    transcript_result, intelligence_result, output_path: Path, filename: str = "report.md"
):
    ...
    md_content.append(
        f"**Provider:** {transcript_result.provider} + {intelligence_result.provider}"
    )
    ...
    if intelligence_result.relationships:
        ...
        if len(intelligence_result.relationships) > 15:
            md_content.append(
                f"*Showing top 15 of {len(intelligence_result.relationships)} relationships*\n"
            )
