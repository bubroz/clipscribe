...
def export_to_pdf(
    ...
    if metadata:
        if "cost" in metadata:
            story.append(
                Paragraph(f"<b>Processing Cost:</b> ${metadata['cost']:.4f}", styles["Normal"])
            )
        story.append(
            Paragraph(
                f"<b>Generated:</b> {datetime.now().strftime('%Y-%m-%d %H:%M')}", styles["Normal"]
            )
        )
    ...
    for entity in entities[:20]:  # Top 20
        ...
        mentions = (
            ...
            else getattr(entity, "mention_count", 0)
        )
