...
def generate_docx_report(
    ...
    if intelligence_result.entities:
        ...
        if len(intelligence_result.entities) > 20:
            doc.add_paragraph()
            note = doc.add_paragraph(f"Note: Showing top 20 of {len(intelligence_result.entities)} entities. Full data in CSV/JSON.")
    ...
    doc.add_paragraph(f"Total: {len(intelligence_result.relationships)} relationships with evidence")
    ...
    if intelligence_result.relationships:
        ...
        if len(intelligence_result.relationships) > 15:
            doc.add_paragraph()
            note = doc.add_paragraph(f"Note: Showing top 15 of {len(intelligence_result.relationships)} relationships. Full data in CSV/JSON.")
            note.runs[0].italic = True
