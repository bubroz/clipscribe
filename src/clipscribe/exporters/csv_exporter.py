...
def export_entities_csv(entities: List, output_path: Path):
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=["name", "type", "confidence", "mention_count", "extraction_source"]
        )
        writer.writeheader()
...
def export_relationships_csv(relationships: List, output_path: Path):
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=["subject", "predicate", "object", "confidence", "source"]
        )
        writer.writeheader()
                ...
                writer.writerow(
                    {
                        ...
                        "confidence": (
                            getattr(rel, "confidence", 0.9) if hasattr(rel, "confidence") else 0.9
                        ),
