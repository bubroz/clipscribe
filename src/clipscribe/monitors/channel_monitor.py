...
class ChannelMonitor:
    ...
    def __init__(
        ...
        logger.info(f"ChannelMonitor initialized: {len(channel_ids)} channels, {len(self.seen_videos)} videos seen")
    ...
    async def check_for_new_videos(self) -> List[Dict]:
        ...
        if new_videos:
            self._save_state()
            logger.info(f"Found {len(new_videos)} new videos across {len(self.channel_ids)} channels")
