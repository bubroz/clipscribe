...
class Evidence(BaseModel):
    ...
    source: str = Field(default="grok-4-1-fast-reasoning")
...
class Entity(BaseModel):
    ...
    extraction_sources: List[str] = Field(default_factory=lambda: ["grok-4-1-fast-reasoning"])
...
class Relationship(BaseModel):
    ...
    confidence: float = Field(default=0.9, ge=0.0, le=1.0)
    evidence: List[Evidence] = Field(default_factory=list)
    extraction_source: str = Field(default="grok-4-1-fast-reasoning")
