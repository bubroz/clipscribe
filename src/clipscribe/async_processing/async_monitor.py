...
class AsyncMonitorOrchestrator:
    ...
    def __init__(
        self, channel_ids: List[str], max_workers: int = 10, output_dir: str = "output/monitored"
    ):
        """
        ...
        self.worker_pool = VideoWorkerPool(max_workers=max_workers, output_dir=output_dir)
        self.channel_monitor = ChannelMonitor(channel_ids)

        self.running = False

        logger.info(
            f"AsyncMonitorOrchestrator initialized: {len(channel_ids)} channels, {max_workers} workers"
        )
                ...
                try:
                    ...
                    if dashboard["currently_processing"]:
                        ...
                        if len(dashboard["currently_processing"]) > 3:
                            processing_titles += (
                                f" +{len(dashboard['currently_processing']) - 3} more"
                            )
