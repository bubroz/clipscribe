...
class OutputValidator:
    ...
    def _validate_core_json(self, data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            empty_evidence_count = sum(1 for entity in core_data.entities if not entity.evidence)
            if empty_evidence_count > 0:
                report["warnings"].append(f"{empty_evidence_count} entities have no evidence")

            # Check mention counts
            suspicious_mentions = [
                e for e in core_data.entities if e.mention_count == 1 and len(e.evidence) > 1
            ]
    ...
    def _validate_graph_json(self, data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if disconnected:
                report["warnings"].append(f"{len(disconnected)} disconnected nodes in graph")
    ...
    def _validate_text(self, filepath: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(content) < 100:
                report["warnings"].append(
                    f"{filepath.name} is suspiciously short ({len(content)} chars)"
                )
