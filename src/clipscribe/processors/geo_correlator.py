...
from ..utils.klv.geometry import calculate_look_vector
...
class GeoCorrelator:
    ...
    def _find_point_at_time(self, target_micros: float) -> Optional[Dict]:
        ...
        if abs(p1["PrecisionTimeStamp"] - target_micros) < abs(
            p2["PrecisionTimeStamp"] - target_micros
        ):
            return p1
        else:
            return p2
    ...
    def _build_geo_context(self, point: Dict) -> Dict:
        ...
        if context["sensor"]["lat"] is not None and context["target"]["lat"] is not None:
