...
class ChunkedIntelligenceExtractor:
    ...
    async def extract_intelligence(self, transcript_text: str, metadata: dict) -> dict:
        ...
        if needs_chunking:
            logger.info(f"Long transcript detected ({len(transcript_text)} chars), using chunked extraction")
            return await self._extract_intelligence_chunked(transcript_text, metadata, max_chunk_size)
        else:
    ...
    async def _extract_intelligence_single(self, transcript_text: str, metadata: dict) -> dict:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=300) as client:
                ...
                logger.info(f"Successfully extracted intelligence: {len(result.get('entities', []))} entities, {len(result.get('relationships', []))} relationships")
                return result
                
        except json.JSONDecodeError as e:
            logger.error(f"JSON parsing failed: {e}")
            logger.error(f"Response content (first 500 chars): {content[:500] if 'content' in locals() else 'No content'}")
            return self._create_fallback_result()
        except Exception as e:
            logger.error(f"Grok intelligence extraction failed: {e}")
            return self._create_fallback_result()
    
    async def _extract_intelligence_chunked(self, transcript_text: str, metadata: dict, chunk_size: int) -> dict:
        ...
        logger.info(f"Merged intelligence from {len(chunks)} chunks: {len(merged_result.get('entities', []))} entities, {len(merged_result.get('relationships', []))} relationships")
        
        return merged_result
    
    def _split_transcript_into_chunks(self, text: str, chunk_size: int, overlap: int = 1000) -> list:
        ...
        for result in chunk_results:
            ...
            for topic, score in sentiment.get('by_topic', {}).items():
                if topic in merged['sentiment']['by_topic']:
                    merged['sentiment']['by_topic'][topic] = (merged['sentiment']['by_topic'][topic] + score) / 2
                else:
