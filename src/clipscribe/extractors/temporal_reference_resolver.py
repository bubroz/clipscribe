...
class TemporalReferenceResolver:
    ...
    def resolve_temporal_references(
        ...
        if content_date != publication_date:
            logger.info(f"Content date ({content_date.date()}) differs from publication date ({publication_date.date()})")
        ...
        for pattern in self.temporal_patterns:
            ...
            for match in matches:
                ...
                if resolved:
                    ...
                    resolved.date_source = "content_date" if content_date != publication_date else "publication_date"
        ...
        logger.info(f"Resolved {len(deduplicated)} temporal references from {len(resolved_references)} matches")
    ...
    def _detect_content_date(
        ...
        if explicit_date:
            confidence = 0.9
            logger.info(f"Found explicit content date: {explicit_date.date()} (confidence: {confidence})")
        ...
        if gemini_date:
            confidence = 0.8
            logger.info(f"Using Gemini-extracted content date: {gemini_date.date()} (confidence: {confidence})")
            return gemini_date
        ...
        if contextual_date:
            confidence = 0.6
            logger.info(f"Inferred content date from context: {contextual_date.date()} (confidence: {confidence})")
        ...
        if metadata_date:
            confidence = 0.7
            logger.info(f"Using metadata recording date: {metadata_date.date()} (confidence: {confidence})")
    ...
    def _extract_explicit_content_date(self, transcript_text: str, publication_date: datetime) -> Optional[datetime]:
    ...
    def _get_gemini_content_date(self, video_intel: VideoIntelligence, publication_date: datetime) -> Optional[datetime]:
        ...
        content_indicators = [
            'today', 'this is', 'recording', 'filming', 'broadcast',
            'live', 'current', 'now', 'present'
        ]
        ...
        for date_info in dates:
            ...
            if any(indicator in context or indicator in original_text for indicator in content_indicators):
                try:
                    normalized_date = date_info.get('normalized_date')
                    if normalized_date:
                        content_date = datetime.fromisoformat(normalized_date.replace('Z', '+00:00')).replace(tzinfo=None)
    ...
    def _analyze_temporal_context_clues(self, transcript_text: str, publication_date: datetime) -> Optional[datetime]:
        """Analyze context clues to infer content date."""
        
        # Look for phrases that indicate old content
        archive_indicators = [
            r'archive footage', r'recorded (\w+) (\d{1,2}),? (\d{4})',
            r'filmed in (\d{4})', r'from our (\d{4}) interview',
            r'back in (\d{4})', r'(\d{4}) recording'
        ]
    ...
    def _resolve_match(
        ...
        elif 'last' in reference_text and any(day in reference_text for day in self.weekday_map.keys()):
            return self._resolve_last_weekday(match, reference_date, context, transcript_text)
        
        elif 'next' in reference_text and any(day in reference_text for day in self.weekday_map.keys()):
    ...
    def _resolve_last_weekday(
        ...
        weekday_match = re.search(r'(monday|tuesday|wednesday|thursday|friday|saturday|sunday)', 
    ...
    def _deduplicate_references(self, references: List[TemporalReference]) -> List[TemporalReference]:
