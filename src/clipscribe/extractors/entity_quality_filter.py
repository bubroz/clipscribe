...
class EntityQualityFilter:
        ...
        self.type_validation = {
            'PERSON': {
                'positive_indicators': {
                    ...
                    'common_names': {'john', 'mary', 'james', 'sarah', 'michael', 'david', 'robert'}
                },
                'negative_indicators': {
    ...
    async def filter_and_enhance_entities(
        ...
        step3_entities = await self._calculate_dynamic_confidence(step2_entities, context_text, metrics)
    ...
    def _remove_false_positives(self, entities: List[Entity], metrics: QualityMetrics) -> List[Entity]:
    ...
    def _calculate_entity_quality_score(self, entity: Entity, context_text: Optional[str]) -> EntityQualityScore:
        ...
        for i, word in enumerate(words):
            if entity_name_lower in word:
                ...
                if any(indicator in surrounding for indicator in [
                    'said', 'announced', 'reported', 'according', 'spokesperson',
                    'president', 'ceo', 'director', 'official', 'minister'
                ]):
                    context_quality += 0.3
    ...
    def _validate_and_correct_types(self, entities: List[Entity], metrics: QualityMetrics) -> List[Entity]:
    ...
    def _suggest_type_correction(self, entity: Entity) -> str:
        ...
        org_indicators = ['inc', 'corp', 'llc', 'ltd', 'company', 'agency', 'department', 'university']
        if any(indicator in name_lower for indicator in org_indicators):
            return 'ORGANIZATION'
        
        # Location indicators
        loc_indicators = ['city', 'state', 'country', 'county', 'district', 'region', 'street', 'avenue']
        if any(indicator in name_lower for indicator in loc_indicators):
            return 'LOCATION'
        
        return entity.type  # No correction needed
    
    def _correct_source_attribution(self, entities: List[Entity], metrics: QualityMetrics) -> List[Entity]:
    ...
    def _apply_final_quality_threshold(self, entities: List[Entity], metrics: QualityMetrics) -> List[Entity]:
