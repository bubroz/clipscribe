...
class EnhancedEntityExtractor:
    ...
    def enhance_entities(
        ...
        for canonical_form, entity_group in entity_groups.items():
            logger.debug(
                f"DEBUG: Processing group '{canonical_form}' with {len(entity_group)} entities"
            )
        ...
        logger.debug(
            f"DEBUG: enhance_entities returning {len(enhanced_entities)} enhanced entities"
        )
    ...
    def _group_entities(self, entities: List[Entity]) -> Dict[str, List[Entity]]:
        ...
        for canonical, group in groups.items():
            ...
            best_canonical = self._select_best_canonical(
                [canonical] + similar_canonicals, merged_group
            )
    ...
    def _extract_temporal_distribution(
        self, entity_group: List[Entity], context_windows: List[EntityContext]
    ...
    def _extract_aliases(self, canonical_form: str, entity_group: List[Entity]) -> List[str]:
