...
from ..models import (
    Relationship, RelationshipEvidence, VideoIntelligence, VideoTranscript, 
    EnhancedEntity, EntityContext
)
...
class RelationshipEvidenceExtractor:
    ...
    def __init__(self):
        ...
        self.action_verbs = {
            'announced', 'declared', 'stated', 'said', 'claimed', 'revealed',
            'signed', 'approved', 'vetoed', 'appointed', 'nominated', 'fired',
            'acquired', 'merged', 'partnered', 'invested', 'funded', 'supported',
            'criticized', 'praised', 'attacked', 'defended', 'met', 'visited',
            'founded', 'created', 'launched', 'developed', 'discovered'
        }
        
        # Visual context indicators
        self.visual_indicators = [
            'shown on screen', 'displayed', 'visible', 'footage shows',
            'video shows', 'image shows', 'graphic shows', 'chart shows',
            'document shows', 'slide shows', 'presentation shows'
        ]
    ...
    def extract_evidence_chains(
        ...
        for relationship in relationships:
            ...
            visual_correlation = self._check_visual_correlation(
    ...
    def _segment_mentions_relationship(
        self, 
        text: str, 
        relationship: Relationship
    ) -> bool:
    ...
    def _extract_quotes_from_segment(
        self, 
        text: str, 
        relationship: Relationship
    ) -> List[str]:
    ...
    def _extract_key_phrase(
        ...
        for sentence in sentences:
            ...
            if (relationship.predicate.lower() in sentence_lower or
                any(verb in sentence_lower for verb in self.action_verbs)):
                return sentence.strip()
    ...
    def _check_visual_correlation(
        ...
        for segment in video_intel.transcript.segments:
            ...
            if (self._segment_mentions_relationship(text, relationship) and
                any(indicator in text.lower() for indicator in self.visual_indicators)):
                return True
