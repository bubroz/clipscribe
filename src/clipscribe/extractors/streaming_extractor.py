...
class StreamingExtractor:
    ...
    async def extract_from_segments(
        ...
        chunk_results = await asyncio.gather(*[process_chunk_with_limit(chunk) for chunk in chunks])

        # Aggregate results
        return self._aggregate_results(chunk_results, total_duration)

    def _create_chunks(
        self, segments: List[Dict[str, Any]], chunk_duration: int
    ...
    async def _process_chunk(self, chunk_data: Tuple[List[Dict], float, float]) -> ChunkResult:
...
"""
    ...
    def _aggregate_results(
        self, chunk_results: List[ChunkResult], total_duration: int
    ) -> Dict[str, Any]:
