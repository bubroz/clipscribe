...
class SeriesDetector:
    ...
    def __init__(self, similarity_threshold: float = 0.7):
        ...
        self.continuation_words = {
            'continues', 'continued', 'continuation', 'part', 'episode', 'chapter',
            'follow-up', 'followup', 'sequel', 'next', 'previous', 'update',
            'revisited', 'extended', 'expanded', 'deep dive', 'analysis'
        }
    ...
    async def detect_series(self, videos: List[VideoIntelligence]) -> SeriesDetectionResult:
        ...
        detection_result = self._combine_analyses(
            videos, title_analysis, temporal_analysis, 
            content_analysis, channel_analysis
        )
        
        logger.info(f"Series detection complete: {detection_result.is_series} (confidence: {detection_result.confidence:.2f})")
    ...
    def _check_sequential_pattern(self, matches: List[Dict]) -> float:
        ...
        sequential_score = sum(1 for i, num in enumerate(numbers) if i < len(expected_sequence) and num == expected_sequence[i]) / len(numbers)
        ...
        for i in range(1, len(titles)):
            ...
            if matching_blocks:
                ...
                if longest_match.size > 10:  # Minimum meaningful length
                    common_base = common_base[longest_match.a:longest_match.a + longest_match.size]
    ...
    def _analyze_temporal_patterns(self, videos: List[VideoIntelligence]) -> Dict[str, Any]:
        ...
        for i in range(1, len(sorted_videos)):
            gap = sorted_videos[i].metadata.published_at - sorted_videos[i-1].metadata.published_at
        ...
        return {
            ...
            'publication_span_days': (sorted_videos[-1].metadata.published_at - sorted_videos[0].metadata.published_at).total_seconds() / 86400
        }
    ...
    async def _calculate_video_similarity(self, video1: VideoIntelligence, video2: VideoIntelligence) -> VideoSimilarity:
        ...
        time_diff = abs((video1.metadata.published_at - video2.metadata.published_at).total_seconds())
        ...
        title_similarity = SequenceMatcher(
            None, 
            video1.metadata.title.lower(), 
            video2.metadata.title.lower()
        ).ratio()
    ...
    def _analyze_channel_consistency(self, videos: List[VideoIntelligence]) -> Dict[str, Any]:
        ...
        channel_score = 1.0 if same_channel else (dominant_channel[1] / len(videos) if dominant_channel else 0.0)
    ...
    def _combine_analyses(
        ...
        detection_method = "+".join(detection_methods) if detection_methods else "insufficient_evidence"
    ...
    def _cluster_by_similarity(self, video_ids: List[str], similarities: List[VideoSimilarity]) -> List[List[str]]:
    ...
    def create_series_metadata(
        ...
        if len(videos) > 1:
            ...
            if base_analysis['has_common_base']:
                ...
                base_title = re.sub(r'\s*(part|pt\.?|episode|ep\.?)\s*\d+.*$', '', titles[0], flags=re.IGNORECASE).strip()
        
        series_title = base_title or f"{first_video.metadata.channel} Video Series"
        
        # Detect pattern
        pattern = detection_result.series_patterns[0] if detection_result.series_patterns else "content_similarity"
