...
class MetadataExtractor:
    ...
    def _extract_klv(self, video_path: str) -> List[Dict]:
        ...
        cmd = [
            'ffmpeg',
            '-i', video_path,
            '-map', '0:1',  # Data stream usually 0:1 or 0:d
            '-codec', 'copy',
            '-f', 'data',
            '-'
        ]
    ...
    def correlate_with_transcript(self, metadata: List[Dict], transcript_segments: List[Dict]) -> List[Dict]:
        """
        ...
        if mode == "absolute":
            ...
            timestamps = [m.get('PrecisionTimeStamp') for m in metadata if 'PrecisionTimeStamp' in m]
            if not timestamps:
        ...
        for segment in transcript_segments:
            ...
            if mode == "absolute":
                ...
                points = [
                    m for m in metadata 
                    if 'PrecisionTimeStamp' in m 
                    and abs_start <= m['PrecisionTimeStamp'] <= abs_end
                ]
            else:
                ...
                points = [
                    m for m in metadata
                    if 'video_time' in m
                    and seg_start <= m['video_time'] <= seg_end
                ]
