...
class VideoIntelligenceRetrieverV2:
    ...
    async def process_local_file(
        ...
        try:
            ...
            result = await self.processor.process_video(
                str(audio_path), metadata.__dict__ if hasattr(metadata, "__dict__") else metadata
            )
    ...
    async def process_url(
        self, video_url: str, force_reprocess: bool = False
    ) -> Optional[VideoIntelligence]:
        """
    ...
    async def _download_video(
        self, video_url: str
    ) -> tuple[Optional[Path], Optional[VideoMetadata], Optional[Path], Optional[Path]]:
    ...
    def _save_outputs(self, result: VideoIntelligence) -> Dict[str, Any]:
            ...
            entities_dict.append(
                {
                    ...
                    "extraction_sources": getattr(
                        entity, "extraction_sources", ["hybrid_processor"]
                    ),
        ...
        core_data = CoreData(
            ...
            transcript_segments=[
                {
                    "text": (
                        ...
                        if hasattr(result.transcript, "text")
            ...
            topics=[
                str(topic) if hasattr(topic, "name") else str(topic)
                for topic in (result.topics if hasattr(result, "topics") else [])
            ],
        ...
        video_id = (
            ...
            if hasattr(result.metadata, "video_id") and result.metadata.video_id
            else self._extract_video_id(result.metadata.url) or "unknown"
        )
        ...
        if self.settings.export_graph_formats:
            try:
                ...
                # GraphML
                graphml_content = self.kg_builder.generate_graphml_content(
                    core_data.knowledge_graph
                )
    ...
    def save_collection_outputs(self, multi_result, output_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with open(summary_file, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"**Cross-Video Relationships:** {len(multi_result.cross_video_relationships)}\n"
                )
    ...
    async def generate_x_content(
        self, result: VideoIntelligence, output_dir: Path, temp_thumbnail: Optional[Path] = None
