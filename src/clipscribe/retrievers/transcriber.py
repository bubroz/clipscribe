...
class GeminiFlashTranscriber:
    ...
    def _parse_json_response(
        self, response_text: str, expected_type: str = "object"
    ) -> Optional[Any]:
        ...
        if self.use_vertex_ai and self.vertex_transcriber:
            ...
            try:
                result = await self.vertex_transcriber.transcribe_with_vertex(
                    Path(audio_file), enhance_transcript=False, mode="audio"
                )
                return self._convert_vertex_result_to_dict(result)
            except Exception as e:
                logger.warning(
                    f"Vertex AI transcription failed: {e}. Falling back to standard Gemini API."
                )
        ...
        try:
            ...
            if self.performance_monitor:
                transcription_event = self.performance_monitor.start_timer(
                    "gemini_transcription", model=transcription_model.model_name
                )
            ...
            if self.performance_monitor:
                analysis_event = self.performance_monitor.start_timer(
                    "gemini_combined_analysis", model=analysis_model.model_name
                )
            ...
            if len(entities) == 0 and len(transcript_text) > 100:
                ...
                if self.performance_monitor:
                    second_pass_event = self.performance_monitor.start_timer(
                        "gemini_second_pass_analysis", model=second_model.model_name
                    )
                ...
                logger.info(
                    f"Second pass found {len(additional_entities)} more entities and {len(additional_relationships)} more relationships"
                )
            ...
            logger.info(
                f"Extracted: {len(entities)} entities, {len(relationships)} relationships, {len(key_points)} key points"
            )
        ...
        for attempt in range(retries):
            try:
                ...
                if ("503" in str(e) or "500" in str(e)) and attempt < retries - 1:
                    logger.warning(
                        f"Transient error ('{e}'): Retrying in {delay}s (attempt {attempt+1}/{retries})"
                    )
    ...
    async def _upload_file_with_retry(
        self, file_path: str, retries: int = 3, timeout: int = 300
    ) -> Any:
        ...
        for attempt in range(retries):
            ...
            try:
                logger.info(
                    f"Attempt {attempt + 1}/{retries}: Uploading file {Path(file_path).name}..."
                )
                ...
                if attempt < retries - 1:
                    ...
                    logger.critical(
                        f"Upload for {file_path} failed after all retries due to timeout."
                    )
                    raise
            except Exception as e:
                if attempt < retries - 1:
    ...
    async def transcribe_large_video(self, video_file: str, metadata: Any, duration: int) -> Dict[str, Any]:
        ...
        logger.info(
            f"Transcribing {len(video_chunks)} chunks with a concurrency limit of {self.settings.gemini_concurrent_requests}..."
        )
    ...
    @retry(
        ...
        before_sleep=lambda retry_state: logger.warning(
            f"Chunk transcription attempt {retry_state.attempt_number} failed, retrying..."
        ),
    )
    async def _transcribe_chunk_raw(self, chunk_file: str, semaphore: asyncio.Semaphore) -> str:
        ...
        async with semaphore:
            ...
            try:
                ...
                transcript_prompt = (
                    "Transcribe this audio. Provide only the text of the transcript."
                )
    ...
    async def _extract_audio_for_upload(self, video_chunk_path: str) -> str:
        ...
        try:
            ...
            logger.error(
                f"Audio extraction failed: {e.stderr.decode('utf-8', errors='ignore') if hasattr(e, 'stderr') else e}"
            )
