...
class VertexAITranscriber:
            ...
            try:
                ...
                if gcs_uri:
                    try:
                        ...
                        logger.info(
                            "Retrying Vertex generate with inline bytes. Generation config unchanged."
                        )
    ...
    @tenacity.retry(
        stop=tenacity.stop_after_attempt(10), wait=tenacity.wait_exponential(min=5, max=300)
    )
    ...
    def _build_comprehensive_prompt(
        self, enhance_transcript: bool = False, mode: str = "video"
    ) -> str:
...
IMPORTANT: Return ONLY valid JSON, no markdown formatting or additional text."""
    ...
    def _parse_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Error around: ...{normalized[max(0, e.pos-50):min(len(normalized), e.pos+50)]}..."
                )
                return {
                    "transcript": {
            ...
            return {
                ...
                "temporal_intelligence": {
                    "temporal_events": data.get("temporal_intelligence", {}).get(
                        "temporal_events", []
                    ),
                    "visual_timestamps": data.get("temporal_intelligence", {}).get(
                        "visual_timestamps", []
                    ),
                    "dates_mentioned": data.get("temporal_intelligence", {}).get(
                        "dates_mentioned", []
                    ),
                },
                "entities": entities,
    ...
    def _create_fallback_response(self, error_message: str) -> Dict[str, Any]:
        ...
        return {
            "transcript": {"segments": [], "full_text": f"[Error: {error_message}]"},
            "temporal_intelligence": {
                "temporal_events": [],
                "visual_timestamps": [],
                "dates_mentioned": [],
