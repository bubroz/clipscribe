...
class PerformanceMonitor:
    ...
    def end_batch_processing(self):
        ...
        if self.batch_metrics["start_time"]:
            ...
            self.record_metric(
                "batch_total_relationships", self.batch_metrics["total_relationships"]
            )
            ...
            if self.batch_metrics["videos_processed"] > 0:
                ...
                avg_entities_per_video = (
                    self.batch_metrics["total_entities"] / self.batch_metrics["videos_processed"]
                )
    ...
    def get_model_cache_stats(self) -> Dict[str, Any]:
        ...
        all_models = set(self.model_cache_metrics["cache_hits"].keys()) | set(
            self.model_cache_metrics["cache_misses"].keys()
        )
    ...
    def get_batch_stats(self) -> Dict[str, Any]:
        ...
        if self.batch_metrics["videos_processed"] > 0:
            successful_videos = (
                self.batch_metrics["videos_processed"] - self.batch_metrics["error_count"]
            )
        ...
        if total_time > 0:
            stats["throughput"] = {
                ...
                "relationships_per_minute": (self.batch_metrics["total_relationships"] / total_time)
                * 60,
            }
            ...
            with open(summary_path, "w", encoding="utf-8") as f:
                ...
                if cache_stats["total_hits"] + cache_stats["total_misses"] > 0:
                    ...
                    if cache_stats["cache_performance"]:
                        ...
                        for model, perf in cache_stats["cache_performance"].items():
                            ...
                            f.write(
                                f"| {model} | {perf['hit_rate']:.1%} | {perf['hits']} | {perf['misses']} | {avg_load:.2f}s |\n"
                            )
                        f.write("\n")
                ...
                if batch_stats:
                    ...
                    if batch_stats.get("throughput"):
                        ...
                        f.write(
                            f"- **Relationships/minute**: {throughput['relationships_per_minute']:.1f}\n\n"
                        )
