...
class ProcessingTracker:
    ...
    def get_stats(self) -> Dict:
        ...
        in_progress = sum(1 for v in self.db.values() if v.get('status') not in [ProcessingStatus.COMPLETED, ProcessingStatus.FAILED])
    ...
    def get_failed_videos(self) -> list:
        ...
        return [
            vid for vid, data in self.db.items()
            if data.get('status') == ProcessingStatus.FAILED
        ]
