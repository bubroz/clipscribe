...
class AsyncProgressIndicator:
    ...
    async def update_progress_phase(
        ...
        state["progress"].update(
            state["detail_task"],
            completed=progress_percent or 0,
    ...
    async def _show_completion_summary(self):
        ...
        summary.add_row("Processing Speed", f"{60/total_time:.1f}x realtime" if total_time > 0 else "N/A")
        
        # Cost metrics
        summary.add_row("", "")  # Spacer
        summary.add_row("[bold]Cost Analysis[/bold]", "")
        summary.add_row("Total Cost", cost_breakdown["emoji"] + f" ${cost_breakdown['total_cost']:.4f}")
...
async def confirm_expensive_cli_operation(operation: str, cost: float, details: List[str] = None) -> bool:
