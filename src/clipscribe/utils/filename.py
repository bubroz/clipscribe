...
def create_output_filename(title: str, extension: str = "txt", output_dir: str = None) -> Path:
    """
...
def create_structured_filename(
    metadata: Dict[str, Any], format_type: str = "transcript", extension: str = "txt"
) -> str:
    """
...
def extract_video_id_from_url(url: str) -> Optional[str]:
    ...
    # YouTube
    youtube_patterns = [r"(?:v=|/)([0-9A-Za-z_-]{11})(?:\?|&|$)", r"youtu\.be/([0-9A-Za-z_-]{11})"]
...
def create_output_structure(metadata: Dict[str, Any], base_dir: str = None) -> Dict[str, Path]:
    """
    ...
    video_id = extract_video_id_from_url(metadata.get("url", "")) or generate_short_id(
        metadata.get("title", "")
    )
