...
class ErrorHandler:
    ...
    def categorize_error(error: Exception) -> Tuple[str, str, str]:
        ...
        if "private" in error_str or "unavailable" in error_str or "not found" in error_str:
            return "🔒", "AccessError", "Video unavailable. It may be private, deleted, or geo-blocked."
        
        # YouTube bot detection
        if "bot" in error_str or "sign in" in error_str or "cookies" in error_str:
            return "🤖", "BotDetectionError", "YouTube bot detection. Try downloading the video file directly."
        ...
        if "transcription" in error_str or "voxtral" in error_str:
            return "🎤", "TranscriptionError", "Transcription failed. Audio may be corrupted or unsupported."
        
        if "extraction" in error_str or "grok" in error_str or "entity" in error_str:
            return "🧠", "ExtractionError", "Intelligence extraction failed. Try again or check logs."
