...
class SegmentSearcher:
    ...
    def search_phrase(self, phrase: str, max_gap: int = 3) -> List[Dict]:
        ...
        for i in range(len(self.segments)):
            ...
            segment_text = self.segments[i].get('text', '').lower()
            ...
            for j in range(i, min(i + max_gap, len(self.segments))):
                ...
                if not remaining_words:
                    ...
                    results.append({
                        ...
                        'timestamp': self._format_timestamp(self.segments[i].get('start_time', 0))
                    })
                    break
    ...
    def get_context(self, segment_index: int, before: int = 1, after: int = 1) -> Dict:
        ...
        return {
            ...
            'full_text': ' '.join(
                s.get('text', '') for s in self.segments[start_idx:end_idx]
            )
        }
...
class SubtitleExporter:
    ...
    @staticmethod
    def to_youtube_chapters(segments: List[Dict[str, any]], min_chapter_duration: float = 10.0) -> str:
        """
