...
class PerformanceDashboard:
    ...
    def _render_model_cache_dashboard(self):
        ...
        with col3:
            ...
            st.metric(
                "Total Accesses", 
                total_accesses,
                help="Total number of model access requests"
            )
        ...
        if cache_info['cached_models']:
            ...
            for model in cache_info['cached_models']:
                ...
                model_data.append({
                    ...
                    'Time Saved (s)': f"{load_time * (access_count - 1):.2f}" if access_count > 1 else "0.00"
                })
    ...
    def _render_batch_processing_dashboard(self):
        ...
        if 'performance_monitor' not in st.session_state or not st.session_state.performance_monitor:
            st.info("No active batch processing session. Start a batch operation to see metrics here.")
            return
        ...
        with col4:
            ...
            st.metric(
                "Total Time", 
                f"{total_time:.1f}s",
                help="Total time spent on batch processing"
            )
        ...
        if st.session_state.get('research_in_progress', False):
            ...
            if batch_progress:
                progress_value = batch_progress.get('completed', 0) / max(batch_progress.get('total', 1), 1)
    ...
    def _render_historical_reports_dashboard(self):
        ...
        if not report_files:
            st.info("No historical performance reports found. Run some batch operations to generate reports.")
            return
    ...
    def _generate_summary_report(self, report_files: List[Path]) -> Dict[str, Any]:
        ...
        if success_rates:
            summary['aggregate_stats']['average_success_rate'] = sum(success_rates) / len(success_rates)
        
        if cache_hit_rates:
            summary['aggregate_stats']['average_cache_hit_rate'] = sum(cache_hit_rates) / len(cache_hit_rates)
