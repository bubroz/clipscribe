...
class VoxtralChunker:
    ...
    def calculate_optimal_chunking(self, duration: int) -> Tuple[int, int, int]:
        ...
        logger.info(
            f"Optimal chunking for {duration}s video: {num_chunks} chunks of {self.chunk_limit}s with {overlap}s overlap"
        )
                ...
                chunks.append(
                    {
                        ...
                        "overlap_end": (
                            min(duration, end_time + overlap) if i < num_chunks - 1 else end_time
                        ),
                    }
                )

                logger.info(
                    f"Created chunk {i+1}/{num_chunks}: {start_time:.1f}s - {end_time:.1f}s"
                )
    ...
    def merge_chunk_transcripts(
        self, chunks: List[Dict[str, Any]], remove_overlap: bool = True
    ) -> Dict[str, Any]:
        """
...
class VoxtralBatchProcessor:
    ...
    def __init__(self, model: str = "voxtral-mini-2507", max_concurrent: int = 5):
        """
    ...
    async def process_batch(
        self, audio_paths: List[str], progress_callback: Optional[callable] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                processed_results.append({"error": str(result), "path": audio_paths[i]})
            else:
    ...
    async def _process_with_limit(
        self, audio_path: str, index: int, total: int, progress_callback: Optional[callable]
    ) -> Dict[str, Any]:
