...
class TranscriptResult(BaseModel):
    ...
    provider: str = Field(
        ..., description="Provider name (voxtral, whisperx-modal, whisperx-local)"
    )
...
class IntelligenceResult(BaseModel):
    ...
    cost_breakdown: Dict = Field(
        default_factory=dict, description="Detailed cost breakdown (input, output, cached, savings)"
    )
    cache_stats: Dict = Field(
        default_factory=dict, description="Cache performance stats (hits, misses, savings)"
    )
    metadata: Dict = Field(default_factory=dict, description="Provider-specific metadata")
