...
async def test_cache_stats_tracking(prompt_cache):
    ...
    assert stats["total_savings_usd"] == pytest.approx(
        0.00075, rel=0.1
    )  # Allow 10% tolerance for float precision
...
@pytest.mark.asyncio
@pytest.mark.skip(
    reason="Collections API not yet available in xAI API (feature planned but not released)"
)
async def test_create_and_list_collections(grok_client):
    ...
    collection_result = await grok_client.create_collection(
        name="test-collection", description="Test collection for ClipScribe"
    )
...
async def test_json_schema_response_format(grok_client):
    ...
    response = await grok_client.chat_completion(
        messages=messages, model="grok-4-1-fast-reasoning", response_format=get_entity_schema()
    )
...
def test_cost_calculation_with_caching(grok_client):
    ...
    cost_breakdown = grok_client.calculate_cost(
        input_tokens=1000, output_tokens=200, cached_tokens=500, model="grok-4-1-fast-reasoning"
    )
