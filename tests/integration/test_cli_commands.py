...
# --- Test Constants ---
TEST_VIDEO_URL = (
    "https://www.youtube.com/watch?v=jNQXAC9IVRw"  # "Me at the zoo" - short and reliable
)
...
def _auth_configured() -> bool:
    ...
    try:
        ...
        if os.environ.get("CLIPSCRIBE_RUN_E2E_ALLOW_AISTUDIO", "").lower() in ("1", "true", "yes"):
            if getattr(s, "google_api_key", "") and os.environ.get("GOOGLE_API_KEY", "") not in (
                "",
                "your_key_here",
            ):
                return True
...
@pytest.mark.skipif(
    not (_should_run_e2e() and _auth_configured()),
    reason="Requires CLIPSCRIBE_RUN_E2E=1 and configured GOOGLE_API_KEY or Vertex creds",
)
...
def test_collection_series_command():
    ...
    assert (
        unified_gexf.exists()
    ), f"Unified knowledge graph was not created for the series. Contents of dir: {list(collection_subdirs[0].iterdir())}"
