"""
End-to-end tests for Modal GPU pipeline with grok-4-1-fast-reasoning.

These tests call the REAL Modal service and REAL grok-4-1-fast-reasoning API.
...
@pytest.mark.asyncio
@pytest.mark.skip(
    reason="Requires deployed Modal app (station10-transcription) - run manually in CI/CD"
)
async def test_modal_pipeline_complete_intelligence():
    """
    Full E2E test of Modal pipeline with grok-4-1-fast-reasoning.

    Tests:
    1. GCS upload
    2. Modal transcription (WhisperX + diarization)
    3. grok-4-1-fast-reasoning intelligence extraction (entities, relationships, topics, moments, sentiment)
    ...
    test_audio = (
        project_root / "test_videos/U3w93r5QRb8_The View Full Broadcast – October 14, 2025.mp3"
    )
    ...
    result_dict = Station10Transcriber().transcribe_from_gcs.remote(
        gcs_input=gcs_url, gcs_output="gs://clipscribe-validation/validation/e2e_test/output/"
    )
...
async def test_modal_pipeline_long_video():
    ...
    test_audio = (
        project_root
        / "test_videos/IbnrclsPGPQ_Trump Brokers Gaza Peace Deal, National Guard in C.mp3"
    )
