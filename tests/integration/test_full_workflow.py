...
async def test_full_workflow_end_to_end():
    ...
    with patch('clipscribe.retrievers.video_retriever.UniversalVideoClient') as mock_client, \
         patch('clipscribe.retrievers.video_retriever.GeminiFlashTranscriber') as mock_transcriber, \
         patch('clipscribe.retrievers.video_retriever.AdvancedHybridExtractor', create=True) as mock_extractor:
