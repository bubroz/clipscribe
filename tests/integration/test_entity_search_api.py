...
async def test_entity_search_confidence_filtering():
    ...
    request = EntitySearchRequest(
        min_confidence=1.0,  # Only perfect confidence
        limit=500  # Get all entities
    )
    
    response = await search_entities(request)
    
    # Most Grok-4 entities have 1.0 confidence (some filtered by deduplication)
    assert response.total >= 200, f"Expected >=200 entities with 1.0 confidence, got {response.total}"
    
    for entity in response.entities[:50]:
        assert entity.confidence == 1.0, f"Entity {entity.name} has confidence {entity.confidence} != 1.0"
